...
setup_python_env() {
    log_info "Setting up Python environment for intelligence system..."
    ...
    python3 -c "import json, hashlib, datetime" 2>/dev/null || {
        log_warning "Some Python modules may be missing, but continuing with available packages"
    }
...
create_session() {
    ...
    log_info "Creating intelligence session: $session_id"
    ...
    python3 "$LOGGING_ROOT/log-aggregator/psycho_noir_aggregator.py" \
        ...
        log_warning "Failed to create session via aggregator, continuing with file-based approach"
...
capture_log() {
    ...
    log_info "Capturing log for session $session_id from $log_source"
    
    # Add to aggregator
    python3 "$LOGGING_ROOT/log-aggregator/psycho_noir_aggregator.py" \
        add "$session_id" "$temp_log" "$log_source" 2>/dev/null || {
        log_warning "Aggregator unavailable, storing log directly"
...
classify_logs() {
    ...
    log_info "Classifying logs for session: $session_id"
...
EOF
    ...
    log_success "Classification completed: $classification_result"
...
process_intelligence() {
    ...
    log_info "Processing intelligence from classification: $classification_file"
    ...
    python3 "$LOGGING_ROOT/intelligence-engine/psycho_noir_intelligence.py" \
        ...
        log_warning "Intelligence processing failed, creating basic result"
...
EOF
    ...
    log_success "Intelligence processed: $intelligence_result"
...
generate_report() {
    ...
    log_info "Generating consciousness report for session: $session_id"
    ...
    python3 "$LOGGING_ROOT/reporting/psycho_noir_reports.py" \
        ...
        log_warning "Report generation failed, creating basic HTML report"
...
finalize_session() {
    ...
    log_info "Finalizing session: $session_id with status: $status"
    
    # Finalize via aggregator
    python3 "$LOGGING_ROOT/log-aggregator/psycho_noir_aggregator.py" \
        finalize "$session_id" "$status" 2>/dev/null || {
        log_warning "Failed to finalize via aggregator"
    ...
    log_success "Session finalized: $session_id"
}
...
EOF
    ...
    log_success "GitHub outputs configured for session: $session_id"
}
...
main() {
    ...
    case "$command" in
        "init")
            ...
            log_info "Initializing intelligence session: $session_id"
        ...
        "process")
            ...
            log_info "Processing complete intelligence pipeline for session: $session_id"
            ...
            log_success "Intelligence pipeline completed for session: $session_id"
        ...
        "status")
            log_info "Psycho-Noir Intelligence System Status"
        ...
        "test")
            log_info "Testing intelligence system components..."
            ...
            log_success "Intelligence system test completed"
            ;;
